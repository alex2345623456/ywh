...
def main():
    ...
    parser.add_argument("-x", "--requests", type=int, default=1)
    args = parser.parse_args()

    total_batches = math.ceil(args.requests / args.threads)
    ...
    for _ in range(total_batches):
        ...
        batch_size = min(args.threads, args.requests)
        args.requests -= batch_size

        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            futures = [executor.submit(send_request, yip_csrf_token) for _ in range(batch_size)]
