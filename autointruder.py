...
def main():
    ...
    if args.p:
        ...
        if len(payloads) < args.t:
            print("Error: Number of payloads must be at least equal to the number of threads (-t).")
            exit(1)

        # Create a cycle of payloads to ensure each thread uses a unique payload
